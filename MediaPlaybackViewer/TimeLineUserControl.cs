...
namespace MediaPlaybackViewer
{
	public partial class TimeLineUserControl : UserControl
	{
		...
		public Item Item
		{
			...
			set { 
				...
				if (value != null)
				{
					...
					{
						if (dt.Id == VideoOS.Platform.Data.DataType.SequenceTypeGuids.RecordingSequence)
							_isRecordingSequencesSupported = true;
						if (dt.Id == VideoOS.Platform.Data.DataType.SequenceTypeGuids.MotionSequence)
		...
		public void SetShowTime(DateTime currentTime)
		{
			if (!_isMouseDown)
			{
				_timeBeforeMouseMove = currentTime.ToLocalTime();
		...
		{
			if (_timeBeforeMouseMove.Ticks == 0)
				return;
			...
			if (_isRequestInProgress == false && _isMouseDown == false)
			{
				if (_startTime < _minRequested)
				{
					...
					if (_isRecordingSequencesSupported)
                        ...
                        _dataSource.GetDataAsync(_syncState, _startTime.ToUniversalTime(), TimeSpan.FromSeconds(0), 0, ts, 300,
                                             VideoOS.Platform.Data.DataType.SequenceTypeGuids.TimelineRecording,
                                             new AsyncCallbackHandler(SequenceRecordingsHandler));
		...
		private void SequenceHandler(object objects, object syncState)
		{
			...
			else
			{
				...
				foreach (SequenceData sd in result)
				{
					...
					Debug.WriteLine("MOTION:" + start.ToLongTimeString() + "..." + end.ToLongTimeString());
				}
		...
		private void OnMouseMove(object sender, MouseEventArgs e)
		{
			if (_isMouseDown)
			{
				...
				if (MouseSetTimeEvent != null)
					MouseSetTimeEvent(this, new TimeEventArgs() {Time = _currentTime.ToUniversalTime()});
			}
